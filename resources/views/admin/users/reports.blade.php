...
@section('title', 'Laporan Harian — ' . ($user->name ?? 'User'))
...
<div class="min-h-screen bg-[#F4F8F6] p-4 sm:p-6 lg:p-7">
    ...
    <div class="mb-6 flex items-center gap-3">
        ...
        <div>
            ...
            <h1 class="text-xl font-extrabold tracking-tight text-[#1B3A34]">Laporan Harian</h1>
        </div>
    </div>

    {{-- Subpage nav --}}
    <div class="mb-5 flex flex-wrap gap-2">
        <span class="rounded-[8px] bg-[#2D8659] px-3 py-1.5 text-[12.5px] font-semibold text-white">Laporan Harian</span>
    ...
    <form method="GET" class="mb-5 rounded-[12px] border border-[#DCE7E1] bg-white p-4 shadow-sm">
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <div class="lg:col-span-2">
                <label class="mb-1 block text-[11.5px] font-semibold text-[#4B5F5A]">Cari aktivitas / tantangan</label>
                <input type="text" name="q" value="{{ request('q', $filters['q'] ?? '') }}"
            ...
            <div>
                ...
                <input type="date" name="from" value="{{ request('from', $filters['from'] ?? '') }}"
                    class="w-full rounded-[8px] border border-[#DCE7E1] bg-[#F4F8F6] px-3 py-2 text-[13px] text-[#1B3A34] outline-none focus:border-[#2D8659]">
            </div>
            <div>
                <label class="mb-1 block text-[11.5px] font-semibold text-[#4B5F5A]">Sampai Tanggal</label>
                <input type="date" name="to" value="{{ request('to', $filters['to'] ?? '') }}"
                    class="w-full rounded-[8px] border border-[#DCE7E1] bg-[#F4F8F6] px-3 py-2 text-[13px] text-[#1B3A34] outline-none focus:border-[#2D8659]">
            </div>
        </div>
            ...
            <table class="w-full min-w-[600px] text-left text-sm">
                <thead>
                    <tr>
                        ...
                        <th class="bg-[#1B3A34] px-5 py-3 text-[11px] font-bold uppercase tracking-[0.06em] text-white">Aktivitas</th>
                        <th class="bg-[#1B3A34] px-5 py-3 text-[11px] font-bold uppercase tracking-[0.06em] text-white">Tantangan</th>
                ...
                <tbody class="divide-y divide-[#DCE7E1]">
                    ...
                    <tr class="transition hover:bg-[#F4F8F6]">
                        ...
                        <td class="px-5 py-4 text-[13px] text-[#4B5F5A]">
                            {{ Str::limit($r->activities, 120) }}
                        </td>
                        <td class="px-5 py-4 text-[13px] text-[#4B5F5A]">
                            {{ Str::limit($r->challenges, 120) }}
                        </td>
